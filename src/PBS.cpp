...
// agents from curr to start also, do the same for ll_min_f_vals and paths_costs
// (since its already "on the way").
...
void PBS::copy_conflicts(const list<Conflict> &conflicts, list<Conflict> &copy, const vector<bool> &excluded_agents) {
    clock_t t = clock();
...
void PBS::find_conflicts(list<Conflict> &conflicts, int a1, int a2) {
    ...
    if (hold_endpoints) {
        ...
        for (size_t timestep = 0; timestep < min_path_length; timestep++) {
            ...
            else if (timestep < min_path_length - 1 && loc1 == paths[a2]->at(timestep + 1).location &&
                     loc2 == paths[a1]->at(timestep + 1).location) {
                conflicts.emplace_back(a1, a2, loc1, loc2,
                                       timestep + 1); // edge conflict
                return;
            }
        }
        ...
        if (paths[a1]->size() != paths[a2]->size()) {
            ...
            for (size_t timestep = min_path_length; timestep < paths[a2_]->size(); timestep++) {
                ...
                if (loc1 == loc2 && G.types[loc1] != "Magic") {
                    conflicts.emplace_back(a1_, a2_, loc1, -1,
...
void PBS::choose_conflict(PBSNode &node) {
  ...
  for (auto conflict : node.conflicts)
  {
    ...
    else if (new_product == product && std::get<4>(*conflict) <
  std::get<4>(*node.conflict)) // choose the earliest
    {
        node.conflict = conflict;
    }
  }
            ...
            min(std::get<0>(*node.conflict), std::get<1>(*node.conflict)) ||
                (min(std::get<0>(*conflict), std::get<1>(*conflict)) ==
                 min(std::get<0>(*node.conflict), std::get<1>(*node.conflict)) &&
                    ...
                    max(std::get<0>(*node.conflict),
    std::get<1>(*node.conflict)))) node.conflict = conflict;
    }*/
...
bool PBS::find_path(PBSNode *node, int agent) {
    ...
    rt.copy(initial_rt);
    // 先build再使用
    rt.build(paths, initial_constraints, node->priorities.get_reachable_nodes(agent), agent, starts[agent].location);
    ...
    std::cout << "pbs.findpath, agent: " << agent << "\n";
    for (auto &g: goal_locations[agent]) {
        std::cout << "goal location: " << g.first << ", " << g.second << "\n";
    }
...
// return true if the agent keeps waiting at its start location until at least
// timestep
...
bool PBS::generate_child(PBSNode *node, PBSNode *parent) {
    ...
    if (lazyPriority) {
        ...
        copy_conflicts(node->parent->conflicts, node->conflicts,
                       -1); // copy all conflicts
...
void PBS::update_best_node(PBSNode *node) {
    if (node->earliest_collision > best_node->earliest_collision or
        (node->earliest_collision == best_node->earliest_collision && node->f_val < best_node->f_val))
        best_node = node;
}

bool PBS::run(const vector<State> &starts, const vector<vector<pair<int, int>>> &goal_locations, int _time_limit) {
    clear();
    std::cout << "PBS run\n";
    // set timer
    start = std::clock();
    ...
    if (!generate_root_node()) {
        std::cout << "Failed to generate root node\n";
        return false;
    }
    ...
    while (!dfs.empty() && !solution_found) {
        ...
        if (screen == 2)
            std::cout << "Expand Node " << curr->time_generated << " ( cost = " << curr->f_val
                      << " , #conflicts = " << curr->num_of_collisions << " ) on conflict " << curr->conflict
                      << std::endl;
        PBSNode *n[2];
        for (auto &i: n)
            i = new PBSNode();
        ...
        for (auto &i: n) {
            ...
            if (sol) {
                if (screen == 2) {
                    std::cout << "Generate #" << i->time_generated << " with " << i->paths.size() << " new paths, "
                              << i->g_val - curr->g_val << " delta cost and " << i->num_of_collisions << " conflicts "
                              << std::endl;
                }
...
PBS::PBS(const BasicGraph &G, SingleAgentSolver &path_planner) :
    MAPFSolver(G, path_planner), lazyPriority(false), best_node(nullptr) {}
    ...
    for (int a1 = 0; a1 < num_of_agents; a1++) {
        for (int a2 = a1 + 1; a2 < num_of_agents; a2++) {
            ...
            if (!conflict.empty()) {
                ...
                if (loc2 < 0)
                    std::cout << "Agents " << a1 << " and " << a2 << " collides at " << loc1 << " at timestep " << t
                              << std::endl;
                else
                    std::cout << "Agents " << a1 << " and " << a2 << " collides at (" << loc1 << "-->" << loc2
...
// adding new nodes to FOCAL (those with min-f-val*f_weight between the old and
// new LB)
void PBS::update_focal_list() {
    ...
    if (open_head->f_val > min_f_val)
    {
            if (screen == 2)
            {
                    std::cout << "  Note -- FOCAL UPDATE!! from |FOCAL|=" <<
    focal_list.size() << " with |OPEN|=" << open_list.size() << " to |FOCAL|=";
            }
...
void PBS::print_results() const {
    ...
    std::cout << runtime << "," << HL_num_expanded << "," << HL_num_generated << "," << solution_cost << ","
              << min_sum_of_costs << "," << avg_path_length << "," << dummy_start->num_of_collisions << ","
              << runtime_plan_paths << "," << runtime_rt << "," << runtime_get_higher_priority_agents << ","
              << runtime_copy_priorities << "," << runtime_detect_conflicts << "," << runtime_copy_conflicts << ","
              << runtime_choose_conflict << "," << runtime_find_consistent_paths << "," << runtime_find_replan_agents
              << std::endl;
}
...
void PBS::save_results(const std::string &fileName, const std::string &instanceName) const {
    ...
    stats << runtime << "," << HL_num_expanded << "," << HL_num_generated << "," << LL_num_expanded << ","
          << LL_num_generated << "," << solution_cost << "," << min_sum_of_costs << "," << avg_path_length << ","
          << dummy_start->num_of_collisions << "," << instanceName << std::endl;
    stats.close();
}
...
void PBS::get_solution() {
    ...
    for (int k = 0; k < num_of_agents; k++) {
        ...
        solution[k] = path_planner.run(G, starts[k], goal_locations[k], rt,
        h_values); if (solution[k].empty()) cout << "ERROR" << endl; rt.clear();
        LL_num_expanded += path_planner.num_expanded;
        LL_num_generated += path_planner.num_generated;*/
