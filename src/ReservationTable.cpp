...
void ReservationTable::mergeIntervals(list<Interval> &intervals) const {
  ...
  while (curr != intervals.end()) {
    if (std::get<1>(*prev) == std::get<0>(*curr) &&
        std::get<2>(*prev) == std::get<2>(*curr)) {
      *prev = make_tuple(std::get<0>(*prev), std::get<1>(*curr),
                         std::get<2>(*prev));
...
void ReservationTable::insertConstraint2SIT(int location, int t_min,
                                            int t_max) {
  if (sit.find(location) == sit.end()) {
    if (t_min > 0) {
...
void ReservationTable::insertSoftConstraint2SIT(int location, int t_min,
                                                int t_max) {
  if (sit.find(location) == sit.end()) {
    if (t_min > 0) {
  ...
  for (auto it = sit[location].begin(); it != sit[location].end(); it++) {
    if (t_min >= std::get<1>(*it))
      ...
      continue;
    // the interval already has conflicts. No need to update
...
void ReservationTable::insertPath2CT(const Path &path) {
  ...
  while (curr != path.end() && curr->timestep - k_robust <= window) {
    if (prev->location != curr->location) {
      ...
      {
        ct[getEdgeIndex(curr->location, prev->location)].emplace_back(
  ...
  if (curr != path.end()) {
    ...
    if (G.types[prev->location] != "Magic")
      ct[prev->location].emplace_back(prev->timestep - k_robust,
                                      path.back().timestep + 1 + k_robust);
    if (k_robust == 0) // add edge constraint
    {
      ct[getEdgeIndex(path.back().location, prev->location)].emplace_back(
...
void ReservationTable::addInitialConstraints(
    const list<tuple<int, int, int>> &initial_constraints, int current_agent) {
...
void ReservationTable::insertPath2CAT(const Path &path) {
  ...
  if (G.types[path.back().location] != "Magic") {
    while (timestep <
           (int)cat.size()) // assume that the agent waits at its last location
    {
...
void ReservationTable::build(
  ...
  addInitialConstraints(initial_constraints,
                        current_agent); // add initial constraints
...
void ReservationTable::build(
  ...
  for (auto con : hard_constraints) {
    if (std::get<0>(con) == current_agent &&
        std::get<4>(con)) // positive constraint
    {
    ...
    {
      ct[getEdgeIndex(std::get<1>(con), std::get<2>(con))].emplace_back(
          std::get<3>(con), std::get<3>(con) + 1);
    }
  }
...
void ReservationTable::insertConstraints4starts(const vector<Path *> &paths,
                                                int current_agent,
                                                int start_location) {
  for (int i = 0; i < num_of_agents; i++) {
    if (paths[i] == nullptr)
      continue;
    else if (i != current_agent) // prohibit the agent from conflicting with
                                 // other agents at their start locations
    {
      ...
      for (auto state : (*paths[i])) {
        ...
        {
          // The agent waits at its start locations between [appear_time,
          // state.timestep - 1] So other agents cannot use this start location
          // between [appear_time - k_robust, state.timestep + k_robust - 1]
...
list<Interval> ReservationTable::getSafeIntervals(int location, int lower_bound,
                                                  int upper_bound) {
...
// [lower_bound, upper_bound)
list<Interval> ReservationTable::getSafeIntervals(int from, int to,
                                                  int lower_bound,
                                                  int upper_bound) {
  if (lower_bound >= upper_bound)
  ...
  auto safe_edge_intervals =
      getSafeIntervals(getEdgeIndex(from, to), lower_bound, upper_bound);

  list<Interval> safe_intervals;
  auto it1 = safe_vertex_intervals.begin();
  auto it2 = safe_edge_intervals.begin();
  while (it1 != safe_vertex_intervals.end() &&
         it2 != safe_edge_intervals.end()) {
    int t_min = max(std::get<0>(*it1), std::get<0>(*it2));
    int t_max = min(std::get<1>(*it1), std::get<1>(*it2));
    if (t_min < t_max)
      safe_intervals.emplace_back(t_min, t_max,
...
bool ReservationTable::findSafeInterval(Interval &interval, int location,
                                        int t_min) {
  updateSIT(location);
...
bool ReservationTable::isConstrained(int curr_id, int next_id,
                                     int next_timestep) const {
  auto it = ct.find(next_id);
  if (it != ct.end()) {
    for (auto time_range : it->second) {
      if (next_timestep >= time_range.first &&
          next_timestep < time_range.second)
        return true;
    }
  }
...
bool ReservationTable::isConflicting(int curr_id, int next_id,
                                     int next_timestep) const {
  ...
  if (cat[next_timestep][next_id])
    return true;
  // check edge constraints (the move from curr_id to next_id at next_timestep-1
  // is disallowed) which means that res_table is occupied with another agent
  // for [curr_id,next_timestep] and [next_id,next_timestep-1] WRONG!
  else if (curr_id != next_id && cat[next_timestep][curr_id] &&
           cat[next_timestep - 1][next_id])
    return true;
  else
    return false;
}
