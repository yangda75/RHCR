...
LRAStar::LRAStar(const BasicGraph &G, SingleAgentSolver &path_planner)
    : MAPFSolver(G, path_planner), num_expanded(0), num_generated(0) {}

bool LRAStar::run(const vector<State> &starts,
...
Path LRAStar::find_shortest_path(const State &start,
  ...
  struct EqNode {
    bool operator()(const StateTimeAStarNode *n1,
                    const StateTimeAStarNode *n2) const {
      return (n1 == n2) ||
  ...
  double h_val =
      path_planner.compute_h_value(G, start.location, 0, goal_locations);
  ...
  while (!open_list.empty()) {
    ...
    if (curr->state.location == goal_locations[curr->goal_id].first &&
        curr->state.timestep >=
            goal_locations[curr->goal_id]
                .second) // reach the goal location after its release time
    {
      curr->goal_id++;
    ...
    for (const auto &next_state : G.get_neighbors(curr->state)) {
      ...
      double next_g_val =
          curr->g_val +
          G.get_weight(curr->state.location, next_state.location) * travel_time;
      double next_h_val = path_planner.compute_h_value(
          G, next_state.location, curr->goal_id, goal_locations);
      if (next_h_val >= INT_MAX) // This vertex cannot reach the goal vertex
        continue;

      // generate (maybe temporary) node
      auto next =
          new StateTimeAStarNode(next_state, next_g_val, next_h_val, curr, 0);
...
void LRAStar::resolve_conflicts(const vector<Path> &input_paths) {
  ...
  for (int t = 1; t <= simulation_window; t++) {
    ...
    for (auto agent : agents_list) {
      ...
      if (loc ==
          solution[agent][t - 1]
              .location) { // The agent wait or rotates at its current location
        ...
        if (other != next_locations.end()) {
          wait_command(other->second, t,
                       path_pointers); // Other agent has to wait
          path_pointers[other->second]--;
        }
      } else if (curr_locations.find(loc) !=
                 curr_locations.end()) { // The agent cannot move because its
                                         // next location is occupied currently
      ...
      } else // The agent wants to move to a location that is currently empty
      {
        ...
        if (other ==
            next_locations
                .end()) { // No other agents want to go to this location yet
          solution[agent].emplace_back(loc, t, orientation);
          path_pointers[agent]++;
        } else { // Another agent also wants to go to this location, for now we
...
void LRAStar::wait_command(
  ...
  auto other =
      next_locations.find(location); // whether conflict with other agent
  if (other != next_locations.end()) {
    wait_command(other->second, timestep,
                 traj_pointers); // Other agent has to wait
...
void LRAStar::print_results() const {
  if (num_wait_commands > 0)
    std::cout << "LRA*:Succeed," << runtime << "," << num_wait_commands << ","
              << num_expanded << "," << num_generated << std::endl;
}

void LRAStar::save_results(const string &fileName,
                           const string &instanceName) const {
  std::ofstream stats;
  stats.open(fileName, std::ios::app);
  stats << runtime << "," << num_wait_commands << "," << num_wait_commands
        << "," << num_expanded << "," << num_generated << "," << 0 << "," << 0
        << "," << 0 << "," << 0 << "," << instanceName << std::endl;
  stats.close();
}
