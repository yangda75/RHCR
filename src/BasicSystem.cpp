...
/*bool BasicSystem::load_config(std::string fname)
{
    ...
    if (!myfile.is_open()) {
        std::cout << "Config file " << fname << " does not exist. " <<
std::endl; return false;
    ...
    getline(myfile, line);
    tok.assign(line, sep);
    double a_max = atoi((*beg).c_str()) * duration * duration / fiducial /
1000000; getline(myfile, line); tok.assign(line, sep); double rotate90 =
atoi((*beg).c_str()) / duration; getline(myfile, line); tok.assign(line, sep);
...
bool BasicSystem::read_MAPF_instance(std::string fname)
{
    ...
    std::ifstream myfile(fname.c_str());
    if (!myfile.is_open()) {
        std::cout << "MAPF instance file " << fname << " does not exist. " <<
std::endl; return false;
    }
...
{
        bool sol = pbs.run(starts, goal_locations, time_limit, vector<Path>(),
PriorityGraph()); pbs.save_results(outfile, std::to_string(num_of_drives) + ","
+ std::to_string(seed));
...
void BasicSystem::update_start_locations() {
  for (int k = 0; k < num_of_drives; k++) {
    starts[k] =
        State(paths[k][timestep].location, 0, paths[k][timestep].orientation);
  }
}

void BasicSystem::update_paths(const std::vector<Path *> &MAPF_paths,
                               int max_timestep = INT_MAX) {
  for (int k = 0; k < num_of_drives; k++) {
    ...
    for (int t = 0; t < length; t++) {
      if (MAPF_paths[k]->at(t).location < 0)
        paths[k][timestep + t] =
            State(-starts[k].location - 1, timestep + t, starts[k].orientation);
      else {
...
void BasicSystem::update_initial_constraints(
    list<tuple<int, int, int>> &initial_constraints) const {
  ...
  for (int k = 0; k < num_of_drives; k++) {
    ...
    for (int t = timestep; t > max(0, timestep - k_robust); t--) {
      ...
      if (curr_location < 0)
        ...
        initial_constraints.emplace_back(k, curr_location,
                                         t + k_robust + 1 - timestep);
  ...
  for (int a1 = 0; a1 < (int)input_paths.size(); a1++) {
    for (int a2 = a1 + 1; a2 < (int)input_paths.size(); a2++) {
      ...
      if ((hold_endpoints || useDummyPaths) &&
        ...
        for (size_t timestep = min_path_length;
             timestep < input_paths[a2_].size(); timestep++) {
...
bool BasicSystem::congested() const {
  ...
  return wait_agents >
         num_of_drives / 2; // more than half of drives didn't make progress
}
...
void BasicSystem::add_partial_priorities(
    const vector<Path> &initial_paths,
    PriorityGraph &initial_priorities) const {
...
void BasicSystem::update_travel_times(
  ...
  for (auto path : paths) {
    ...
    while (t >= t_min) {
      ...
      int wait = 0;
      while (t > wait && path[t - 1 - wait].location == loc &&
...
void BasicSystem::solve() {
  ...
  } else // PBS or ECBS
  {
    ...
    if (hold_endpoints || useDummyPaths) {
      ...
      if (sol) {
        if (log)
          solver.save_constraints_in_goal_node(
              outfile + "/goal_nodes/" + std::to_string(timestep) + ".gv");
        update_paths(solver.solution);
      } else {
        lra.resolve_conflicts(solver.solution);
        update_paths(lra.solution);
    ...
    if (log)
      solver.save_search_tree(outfile + "/search_trees/" +
                              std::to_string(timestep) + ".gv");
  }
  std::cout << timestep << ", " << num_of_drives << "," << seed << std::endl;
  // solver.save_results(outfile + "/solver.csv", std::to_string(timestep) + ","
  //    								+
  //    std::to_string(num_of_drives) + "," + std::to_string(seed));
}

bool BasicSystem::solve_by_WHCA(
    vector<Path> &planned_paths, const vector<State> &new_starts,
    const vector<vector<pair<int, int>>> &new_goal_locations) {
  ...
  if (whca.hold_endpoints) {
    if (timestep == 0) {
      for (int i = 0; i < (int)new_starts.size(); i++)
        whca.initial_solution[i].emplace_back(
            starts[i]); // hold initial location
    } else {
      whca.initial_solution.clear();
      for (auto agent : new_agents)
        whca.initial_solution.emplace_back(
            planned_paths[agent]); // hold old paths
    }
  }
  bool sol = false;
  if (timestep == 0) {
...
bool BasicSystem::load_records() {
  ...
  // pick the timestep
  timestep = int((timestep - 1) / simulation_window) *
             simulation_window; // int((timestep - 1) / simulation_window) *
                                // simulation_window;
