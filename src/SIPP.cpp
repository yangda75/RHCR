...
Path SIPP::updatePath(const BasicGraph &G, const SIPPNode *goal) {
  ...
  while (true) {
    if (curr->parent == nullptr) // root node
    {
      ...
      int degree =
          G.get_rotate_degree(prev->state.orientation, curr->state.orientation);
      ...
      } else if (degree == 2) // turn back
      {
        path[t] = State(prev->state.location, t,
                        (prev->state.orientation + 1) % 4); // turn right
        t++;
        path[t] = State(prev->state.location, t,
                        curr->state.orientation); // turn right
        t++;
      }
...
Path SIPP::run(const BasicGraph &G, const State &start,
  ...
  std::cout << "h value : " << start.location << ", goal : ";
  ...
  if (std::get<0>(interval) == 0) {
    std::cout << "start interval lb is 0\n";
    ...
    node->focal_handle = focal_list.push(node);
                               ...
                               // start location
  {
    // This is correct only when k_robust <= 1. Otherwise, agents might not be
    // able to wait at its start locations due to initial constraints caused by
    // the previous actions of other agents.
  ...
  while (!focal_list.empty()) {
    ...
    {
      if (!G.valid_move(curr->state.location,
                        orientation)) // the edge is blocked
        continue;
      int degree;
      ...
      if (degree > std::get<1>(curr->interval) -
                       curr->state.timestep) // don't have enough time to turn
        continue;
      ...
      for (auto interval :
           rt.getSafeIntervals(curr->state.location, location, min_timestep,
                               std::get<1>(curr->interval) + 1)) {
        if (curr->state.orientation < 0)
          generate_node(interval, curr, G, location, min_timestep, -1, h_val);
        else
          generate_node(interval, curr, G, location, min_timestep, orientation,
                        h_val);
      }
    ...
    {
      if (prioritize_start) // the agent has the highest priority at its start
                            // location
      {
...
void SIPP::generate_node(const Interval &interval, SIPPNode *curr,
  ...
  auto next = new SIPPNode(State(location, timestep, orientation), g_val, h_val,
                           interval, curr, conflicts);
  ...
  next->state.location) * waiting_time); if (waiting_time > 0 && next->getFVal()
  + waiting_cost <= (*it)->getFVal()) { // next arrives later with a smaller
  cost
          // let the following update it with next
      }
      else if (waiting_time < 0 && next->getFVal() >= (*it)->getFVal() +
  waiting_cost) { // next arrives earlier with a larger cost delete next; // so
  delete next return;
      }
  ...
  inserting this node to allNodes_table, it will not override the previous node.
  }*/
  ...
  if (existing_next->in_openlist) { // if its in the open list
    if (existing_f_val > g_val + h_val ||
      ...
      // if f-val decreased through this new path (or it remains the same and
      // there's less internal conflicts)
      bool add_to_focal = false; // check if it was above the focal bound before
                                 // and now below (thus need to be inserted)
      bool update_in_focal =
          false; // check if it was inside the focal and needs to be updated
                 // (because f-val changed)
      bool update_open = false;
      if ((g_val + h_val) <=
          focal_bound) { // if the new f-val qualify to be in FOCAL
        if (existing_f_val > focal_bound)
          add_to_focal = true; // and the previous f-val did not qualify to be
                               // in FOCAL then add
        else
          update_in_focal = true; // and the previous f-val did qualify to be in
                                  // FOCAL then update
      }
      if (existing_f_val > g_val + h_val)
        update_open = true;
      ...
      if (update_open)
        open_list.increase(
            existing_next->open_handle); // increase because f-val improved
      if (add_to_focal)
        existing_next->focal_handle = focal_list.push(existing_next);
      if (update_in_focal)
        focal_list.update(
            existing_next
                ->focal_handle); // should we do update? yes, because number of
                                 // conflicts may go up or down
    }
