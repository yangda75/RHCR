...
public:
    ...
    SIPPNode(const State &state, double g_val, double h_val, const Interval &interval, SIPPNode *parent,
             int conflicts) :
    ...
    struct EqNode {
        bool operator()(const SIPPNode *n1, const SIPPNode *n2) const {
            return (n1 == n2) || (n1 && n2 && n1->state.location == n2->state.location &&
                                  n1->state.orientation == n2->state.orientation && n1->interval == n2->interval &&
...
private:
    ...
    void generate_node(const Interval &interval, SIPPNode *curr, const BasicGraph &G, int location, int min_timestep,
