...
public:
  ...
  list<tuple<int, int, int>>
      initial_constraints; // <agent, location, timestep>:
  // only this agent can stay in this location before this timestep.
  list<const Path *>
      initial_soft_path_constraints; // the paths that all agents try to avoid
  ...
  virtual bool run(const vector<State> &starts,
                   const vector<vector<pair<int, int>>>
                       &goal_locations, // an ordered list of pairs of
  ...
  virtual void
  save_constraints_in_goal_node(const std::string &fileName) const = 0;
