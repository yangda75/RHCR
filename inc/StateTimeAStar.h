...
public:
    ...
    StateTimeAStarNode() :
        g_val(0), h_val(0), parent(nullptr), conflicts(0), depth(0), in_openlist(false), goal_id(0) {}
...
public:
    ...
    void findTrajectory(const BasicGraph &G, const State &start, const vector<pair<int, int>> &goal_locations,
                        const unordered_map<int, double> &travel_times, list<pair<int, int>> &path);
